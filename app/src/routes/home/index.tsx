...
const Home: FunctionalComponent = () => {
    ...
    const addAnnotation = () => {
      ...
      const modifiedCode = replaceAt(from, to, index + '«' + selection.toString() + '»');
    ...
    const restructurify = () => {
      ...
      (annotations as annotationType[]).map((a: any, idx: any) => {
        ...
        const input = document.getElementById('annotation-input-'+a.index.toString()) as any;
        json[2].annotations.push({"index": json[2].annotations.length, "content": a.content, "annotation": input.value})
      });
    ...
    const removeAnnotation = (e: any, index: number) => {
     ...
     console.log("coords sub", coords+2, coords+2+removedAnnoation.content.length);

     const test = textvalue.substring(coords+2, coords+removedAnnoation.content.length+2);
     console.log("coords test", test)
     const modifiedCode = replaceAt(coords, coords+removedAnnoation.content.length+3, test);
     console.log("modifiedCode text value", modifiedCode)
     console.log("modifiedCode code", code)
    ...
    const handleKeyDown = (e: any) => {
      let value = textvalue, selStartPos = e.currentTarget.selectionStart;
    ...
    const listAnnotations = (annotations as annotationType[]).map(a => {
      ...
      if (a) {
        return(
          <div> 
            <li><input onInput={(e) => inputChange(e, a.index)} id={'annotation-input-'+a.index} value={annotation.index === a.index ? annotation.annotation : 'error'} />
            { a.content ? a.index+': '+a.content : 'error' }
            <button onClick={(e) => (removeAnnotation(e, a.index))} id="remove-button" data-index={a.index}>x</button>
            </li>
          </div>
        )
      }
    ...
    return (
      <div class={style.home}>
          ...
          <div class={style.leftside}>
            ...
            <pre>
              <code>
                <textarea onKeyDown={handleKeyDown} value={textvalue} id="code-to-annotate" cols={100} rows={10} onInput={handleChange} />
              </code>
            </pre> 
          

            <button onClick={addAnnotation} id="add-annotation-button">Add annotation</button>
            <button onClick={generate} id="restructurify-button">ReStructurify</button>
            ...
            { rstResult ? <div><h4>Rst</h4><div id ="preview-rst"><pre>{rstResult}</pre></div><button onClick={copyRstToClipboard}>Copy</button></div>  : null }
            { jsonResult ? <div><h4>Json</h4><pre>{JSON.stringify(jsonResult, null, 2) }</pre><button onClick={copyJsonToClipboard}>Copy</button></div>  : null }
          </div>
